...
axs[0, 1].plot(data.index, data['SMA'], label='SMA', linewidth=0.7, color='orange')
axs[0, 1].set_title('SMA')
axs[0, 1].set_xlabel('日付')
axs[0, 1].set_ylabel('価格')
axs[0, 1].grid()

axs[1, 0].plot(data.index, data['EMA'], label='EMA', linewidth=0.7, color='green')
axs[1, 0].set_title('EMA')
axs[1, 0].set_xlabel('日付')
axs[1, 0].set_ylabel('価格')
axs[1, 0].grid()

axs[1, 1].plot(data.index, data['RSI'], label='RSI', linewidth=0.7, color='red')
axs[1, 1].set_title('RSI')
axs[1, 1].set_xlabel('日付')
axs[1, 1].set_ylabel('RSI')
axs[1, 1].grid()

axs[2, 0].plot(data.index, data['MOM'], label='MOM', linewidth=0.7, color='purple')
axs[2, 0].set_title('MOM')
axs[2, 0].set_xlabel('日付')
axs[2, 0].set_ylabel('MOM')
axs[2, 0].grid()

axs[2, 1].plot(data.index, data['MACD'], label='MACD', linewidth=0.7, color='brown')
axs[2, 1].set_title('MACD')
axs[2, 1].set_xlabel('日付')
axs[2, 1].set_ylabel('MACD')
axs[2, 1].grid()

axs[3, 0].plot(data.index, data['ATR'], label='ATR', linewidth=0.7, color='pink')
